  ...
  return {
    ...
    visitor: {
      Program(babelPath, state) {
        ...
        if (state.filename?.includes(path.resolve(__dirname, "../../../core"))) {
          return;
        }
        ...
        babelPath.get("body").forEach((child) => {
          if (child.isImportDeclaration() && child.node.source.value === "@vynn/core/jsx-runtime") {
        ...
        if (!hasLogJsx) {
          const importDecl = t.importDeclaration(
            ...
            t.stringLiteral("@vynn/core/jsx-runtime"),
          );
