  ...
  for (let i = 0; i < children.length; i++) {
    ...
    const run = () => {
      ...
      const disposer = effect(() => {
        ...
        const anchor = isFirstRun ? void 0 : getChildAnchor(parentNode, baseAnchor, i);
        isFirstRun = false;
...
function getChildAnchor(parentNode, baseAnchor, index) {
  if (!baseAnchor) {
    return parentNode.childNodes[index] ?? void 0;
  }
  let current = baseAnchor.previousSibling;
  let steps = index;
  while (current && steps > 0) {
    current = current.previousSibling;
    steps--;
  }
  return current?.nextSibling ?? baseAnchor ?? void 0;
}
...
//# sourceMappingURL=h-blgYSJkW.js.map
