...
function patch(parentNode, oldNodes, newNodes) {
  const maxLength = Math.max(oldNodes.length, newNodes.length);
  for (let i = 0; i < maxLength; i++) {
    const oldNode = oldNodes[i];
    const newNode = newNodes[i];
    oldNodes[i] = newNode;
    if (isNil(oldNode) && isNil(newNode)) continue;
    ...
    if (isNil(oldNode) && !isNil(newNode)) {
      const currentDomNode = parentNode.childNodes[i];
      if (currentDomNode) {
        parentNode.insertBefore(newNode, currentDomNode);
      } else {
  ...
  for (const child of toArray(children)) {
    ...
    const run = () => {
      ...
      const disposer = effect(() => {
        ...
        oldNodes = patch(parentNode, oldNodes, newNodes, baseAnchor);
...
export { Fragment as F, Suspense as S, track as a, mountComponent as b, componentRootNodes as c, onMount as d, effect as e, stopEffect as f, getNode$1 as g, jsx as j, loop as l, memo as m, onDestroy as o, renderChildren as r, state as s, trigger as t, untrack as u };
//# sourceMappingURL=h-D7KNOBCd.js.map
