...
function renderChildren(parentNode, rawChildren, baseAnchor) {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (componentRootNodes.has(child)) parentNode.appendChild(child);
    parentNode.insertBefore(anchor, componentRootNodes.has(child) ? child : null);
    const handler = getSuspenseHandler();
    const run = () => {
      let oldNodes = [];
      let newNodes = [];
      const disposer = effect(() => {
        try {
          newNodes = toArray(getNode$1(child)).flat();
        } catch (error) {
          if (error instanceof Promise) {
            if (handler) {
              handler(error);
...
//# sourceMappingURL=h-DPhcrTkR.js.map
