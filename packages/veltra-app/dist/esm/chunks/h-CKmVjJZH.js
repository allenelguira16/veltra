...
function patch(parentNode, oldNodes, newNodes, insertIndex) {
  const maxLength = Math.max(oldNodes.length, newNodes.length);
  const anchor = insertIndex !== void 0 && componentRootNodes.has(parentNode.childNodes[insertIndex]) ? parentNode.childNodes[insertIndex] : void 0;
...
function renderChildren(parentNode, rawChildren) {
  const cleanups = {
    disposer: [],
    oldNodes: []
  };
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    const run = () => {
      ...
      const disposer = effect(() => {
        ...
        oldNodes = patch(parentNode, oldNodes, newNodes, i);
...
//# sourceMappingURL=h-CKmVjJZH.js.map
