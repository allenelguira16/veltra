...
function getNode(jsxElement, i = -1) {
  ...
  if (typeof jsxElement === "function") {
    return getNode(jsxElement(), i);
  }
  if (Array.isArray(jsxElement)) {
    return jsxElement.map((jsxChild) => getNode(jsxChild, i));
  }
...
function applyProps$1(props) {
  ...
  for (const key in props) {
    ...
    if (key === "ref" && typeof value === "function") ; else if (key === "style") ; else if (key === "disabled" && value) {
...
function renderChildren$1(children) {
  ...
  for (const child of children) {
    if (typeof child === "function") {
      ...
      child.forEach((nested) => transformedChildren.push(renderChildren$1([nested])));
    } else {
...
function renderChildren(parentNode, rawChildren, baseAnchor) {
  ...
  const children = rawChildren instanceof Function ? rawChildren() : rawChildren;
  for (const child of toArray(children)) {
    ...
    const run = () => {
      ...
      const disposer = effect(() => {
        try {
          newNodes = toArray(getNode(child));
        } catch (error) {
...
export { Fragment as F, Suspense as S, mountComponent as a, track as b, componentRootNodes as c, onMount as d, effect as e, stopEffect as f, getNode as g, jsx as j, loop as l, memo as m, onDestroy as o, renderChildren as r, state as s, trigger as t, untrack as u };
//# sourceMappingURL=effect-ugIqGzfD.js.map
