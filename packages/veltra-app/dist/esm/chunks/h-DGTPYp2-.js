...
function patch(parentNode, oldNodes, newNodes, insertBeforeNode) {
  ...
  for (let i = 0; i < maxLength; i++) {
    ...
    if (isNil(oldNode) && !isNil(newNode)) {
      const nextSibling = insertBeforeNode ?? parentNode.childNodes[parentNode.childNodes.length - 1]?.nextSibling ?? null;
...
function renderChildren(parentNode, rawChildren) {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    const anchor = parentNode.childNodes[i];
...
//# sourceMappingURL=h-DGTPYp2-.js.map
