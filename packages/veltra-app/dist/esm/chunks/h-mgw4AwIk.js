  ...
  for (const child of toArray(children)) {
    ...
    const run = () => {
      ...
      const disposer = effect(() => {
        try {
          newNodes.push(...toArray(getNode$1(child)));
          if (newNodes.length && newNodes[0] && componentRootNodes.has(newNodes[0])) {
            if (!anchor) {
              parentNode.appendChild(newNodes[0]);
              anchor = newNodes[0];
            }
          }
        } catch (error) {
        ...
        patch(parentNode, oldNodes, newNodes);
        newNodes.length = 0;
...
//# sourceMappingURL=h-mgw4AwIk.js.map
