...
function renderChildren(parentNode, rawChildren, baseAnchor) {
  const listAnchor = createTargetNode("renderChildren-list");
  parentNode.insertBefore(listAnchor, baseAnchor ?? null);
  let oldNodes = [];
  ...
  const run = () => {
    const disposer = effect(() => {
      ...
      try {
        ...
        newNodes = children.flatMap((child) => toArray(getNode$1(child)));
      } catch (error) {
      ...
      oldNodes = patch(parentNode, oldNodes, newNodes, listAnchor);
    });
    return () => {
      disposer();
      patch(parentNode, oldNodes, [], listAnchor);
      listAnchor.remove();
    };
  };
  return run();
...
//# sourceMappingURL=h-C6_Np0DD.js.map
