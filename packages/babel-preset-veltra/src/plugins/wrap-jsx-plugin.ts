...
export const wrapJsxPlugin = declare((api) => {
  ...
  return {
    name: "veltra-jsx-getter-transform",
    visitor: {
      Program: {
        enter(_, state) {
        ...
        exit(path, state) {
          ...
          path.traverse({
            ImportDeclaration(importPath) {
              ...
              if (source === "@veltra/app/jsx-runtime") {
            ...
            CallExpression(callPath) {
              ...
              const transformedProps = props.properties.map((prop) => {
                ...
                if (!t.isExpression(prop.value)) return prop; // e.g. skip patterns
                if (!t.isExpression(prop.key)) return prop; // 🛑 Fix: ensure key is valid
                if (t.isPrivateName(prop.key)) return prop; // 🛑 Fix: skip private keys
